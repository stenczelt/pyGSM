...
class Penalty_PES(PES):
    ...
    def __init__(self,
        ...
        self.dE = 1000.
        self.sigma = sigma
        print(' PES1 multiplicity: {} PES2 multiplicity: {} sigma: {}'.format(self.PES1.multiplicity,
                                                                              self.PES2.multiplicity, self.sigma))
    ...
    def get_gradient(self, geom, frozen_atoms=None):
        ...
        if self.dE < 0:
            dgrad *= -1
        factor = self.sigma * ((self.dE * self.dE) + 2. * self.alpha * abs(self.dE)) / (
